...
def device_JJ( width = 0.135, bridge_width = 1.0, finger_width = 0.2, JJtype = "manhattan", squid = False, bandage = True, photolitho = False, Pad_gap=20):
    JJ=Device('JJ')
    JJ_half=Device('JJ_half')
    if Pad_gap==0:
        print("Pad_gap is zero. Skip JJ process.")
        return JJ
    else:
        if photolitho:
            ...
            if (JJtype == "mh" or JJtype == "manhattan") and bandage:
                ...
                if Pad_gap>99:
                    finger_width_outer1 = 1.0
                finger_length_outer1 = 3.7+Pad_gap/2 #13.7 ##when gap is 20 um
                ...
                box_outer.movey(finger_length_outer1-13.7)
                ...
                # box_inner1 = JJ_half.add_ref( box_inner )
                # box_inner1.rotate(180)
                #JJ_half.movey((finger_length_outer1-3)/2)
...
def device_DicingMarkers(width=200, length=400, street_width=100, layer=3):
    """
    十字のマーカーから、ダイシングストリート（十字）をくり抜くことで、
    ダイシングラインを「囲う」形状（4つのL字、またはレール状）を作成します。
    """
    DicingMarkers = Device("DicingMarkers")
    ...
    tmp1 = pg.bbox([(-0.5*width, -0.5*length), (0.5*width, 0.5*length)])
    tmp2 = pg.bbox([(-0.5*length, -0.5*width), (0.5*length, 0.5*width)])    
    base_cross = pg.boolean(tmp1, tmp2, 'or')
    
    # 2. ダイシングストリートとなる十字（くり抜く型）
    # ※長さはマーカー全体を確実に横切るため、lengthよりも少し長く設定します
    cut_len = length + 100 
    s_tmp1 = pg.bbox([(-0.5*street_width, -0.5*cut_len), (0.5*street_width, 0.5*cut_len)])
    s_tmp2 = pg.bbox([(-0.5*cut_len, -0.5*street_width), (0.5*cut_len, 0.5*street_width)])
    street_cross = pg.boolean(s_tmp1, s_tmp2, 'or')
    
    # 3. ベースからストリートを引く (base NOT street)
    marker = pg.boolean(base_cross, street_cross, 'not', layer=layer)
